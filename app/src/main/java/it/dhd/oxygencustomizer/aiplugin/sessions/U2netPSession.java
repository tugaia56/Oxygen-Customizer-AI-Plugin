...
public class U2netPSession extends BaseSession {

    public U2netPSession(Context context) throws OrtException, IOException {
        super(context, "u2netp.onnx");
    }

    @Override
    public Bitmap predict(Bitmap image) throws OrtException {
        Log.d("U2netpSession", "Normalizing image");
        ...
        try (OrtSession.Result result = session.run(inputs)) {
            ...
            Log.d("U2netpSession", "Prediction completed");

            Bitmap mask = convertArrayToBitmap(pred, pred.length, pred[0].length);

            Log.d("U2netpSession", "Mask created");
    ...
    private Bitmap convertArrayToBitmap(float[][] pred, int originalWidth, int originalHeight) {
        ...
        Log.d("U2netpSession", "Resizing mask to original image size");
